    ...
    def comments(self,info):
        ...
        for i in linklist:
            ...
            #     text = handle_garbled(j)  # 处理乱码
                ...
                # weibo_footer = re.findall(pattern, str_footer, re.M)
                # up_num += int(weibo_footer[0])
            comment_hot_text.append(l)

        for i, text in enumerate(html1.xpath('//div[@class="c"]/a//text()')):
            if text == "查看更多热门>>":
                comment_url = 'https://weibo.cn/'+html1.xpath('//div[@class="c"]/a/@href')[i]
                comment = handle_html(self.cookie, str(comment_url))
                comment = tostring(comment).decode()
                html = etree.HTML(comment)
                linklist = html.xpath('//span/../@id')
                for i in linklist:
                    l = html.xpath('//div[@id="' + i + '"]//span[@class="ctt"]//text()')
                    comment_hot_text.append(l)
                while len(html.xpath('//div[@id="pagelist"]/form/div/a//text()')) > 0 and html.xpath('//div[@id="pagelist"]/form/div/a//text()')[0] == "下页":
                    comment_url = 'https://weibo.cn/'+html.xpath('//div[@id="pagelist"]/form/div/a/@href')[0]
                    comment = handle_html(self.cookie, str(comment_url))
                    comment = tostring(comment).decode()
                    html = etree.HTML(comment)
        ...
        while len(html1.xpath('//div[@id="pagelist"]/form/div/a//text()')) > 0 and html1.xpath('//div[@id="pagelist"]/form/div/a//text()')[0] == "下页":
            comment_url = 'https://weibo.cn/'+html1.xpath('//div[@id="pagelist"]/form/div/a/@href')[0]
            comment = handle_html(self.cookie, str(comment_url))
            comment = tostring(comment).decode()
            html1 = etree.HTML(comment)
        ...
        return comment_hot_text
    

    def forwards(self,info):
        from lxml.html import  tostring
        from lxml import etree
        id = info.xpath('@id')[0][2:]
        retweet = []

        repost_url = 'https://weibo.cn/repost/'+id
        repost = handle_html(self.cookie, str(repost_url))
        repost = tostring(repost).decode()
        html1 = etree.HTML(repost)
        # 获取所有a标签的href属性
        # pattern = r'\d+'
        # up_num = 0
        retweet.extend(html1.xpath('//div[@class="c"]/text()'))

        while len(html1.xpath('//div[@id="pagelist"]/form/div/a//text()')) > 0 and html1.xpath('//div[@id="pagelist"]/form/div/a//text()')[0] == "下页":
            repost_url = 'https://weibo.cn/'+html1.xpath('//div[@id="pagelist"]/form/div/a/@href')[0]
            repost = handle_html(self.cookie, str(repost_url))
            repost = tostring(repost).decode()
            html1 = etree.HTML(repost)
    ...
    def get_one_weibo(self, info):
        ...
        try:
            ...
            if (not self.filter) or is_original:
                ...
                weibo.comments = self.comments(info)
                weibo.forwards = self.forwards(info)
            else:
                weibo = None
